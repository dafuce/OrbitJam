...
public class WindowObject {
    ...
    public void track(double speed, WindowObject target){
        dist = Math.sqrt(Math.pow(Math.abs(this.x - target.x),2) + Math.pow(Math.abs(this.y - target.y),2));
        xcomp = ((target.x - this.x)/dist)*speed;
        ycomp = ((target.y - this.y)/dist)*speed;
        x+=xcomp;
        y+=ycomp;
    }
