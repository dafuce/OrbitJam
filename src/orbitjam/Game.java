...
public class Game extends JFrame {
    ...
    // State Machine for the orbitjam.Game
    ...
    // "Main" orbitjam.Game class constructor
    Game() throws Exception {
        ...
        // Everything window resolution - related
        ...
        fons = SpriteLoader.fons.getScaledInstance(getWindowHeight(),getWindowHeight(),Image.SCALE_SMOOTH);
        g = SpriteLoader.fons.getGraphics();
    ...
    void spawns(){
        if (Utils.rnd(1, 200) == 200) {
            if(player.getHealth() < 3){
                boxes.add(new Box(true, this));
            } else{
                boxes.add(new Box(false,this));
            }
        }
        int d = Math.max(50 - kills / 10, 10);
        if (gameMS > 8000 && Utils.rnd(1, d) == d) {
    ...
    void moveAction() {
        ...
        for (int i = boxes.size() - 1; i >= 0; i--) {
            if(!player.SHOOT.pressed && player.getMagnetSpeed() > 0){
                boxes.get(i).track(player.getMagnetSpeed(),player);
            } else if (!boxes.get(i).move()) {
                boxes.remove(i);
            }
    ...
    public void repaint(Graphics g) {
        ...
        if (state == State.MENU) {
            ...
            // Paint everything player-related
            player.paint(g);
            if (player.inventory[0] > 0) {
                Utils.drawCenteredString(g, Integer.toString(player.inventory[0]), player.getX() + (double) player.getWidth() / 2, player.getY() + player.getHeight() + 25, Utils.font2, Color.LIGHT_GRAY);
            }
            int barWidth = (int) (((Math.min(System.currentTimeMillis() - player.lastShielded, player.shieldCooldown) / (double) player.shieldCooldown)) * (player.getWidth() + 10));
            Color barColor = barWidth == player.getWidth() + 10 ? Utils.marine : Utils.darkblue;
            g.setColor(barColor);
            g.fillRect(player.getX() - 5, player.getY() + player.getHeight() + 10, barWidth, 5);
            if (player.shielded) {
                g.drawImage(SpriteLoader.shield, player.getX() + player.getWidth() / 2 - SpriteLoader.shield.getWidth() / 2, player.getY() + player.getHeight() / 2 - SpriteLoader.shield.getHeight() / 2, null);
            }
            if (gameMS <= 2000) {
                Utils.drawCenteredString(g, "Hold X to Shoot", player.x + (double) player.width / 2, player.y + 65, Utils.font2, Color.white);
                Utils.drawCenteredString(g, "\u2190 \u2192 \u2191  \u2193", player.x + (double) player.width / 2, player.y - 30, Utils.font1, Color.white);
            }
                else if(gameMS <= 4000){
                Utils.drawCenteredString(g, "Release X for Magnet", player.x + (double) player.width / 2, player.y + 65, Utils.font2, Color.white);
            } else if(gameMS <= 6000) {
                Utils.drawCenteredString(g, "Hold C to Shield", player.x + (double) player.width / 2, player.y + 65, Utils.font2, Color.white);
            } else if(gameMS <= 8000){
                Utils.drawCenteredString(g, "Tap C to drop Bomb", player.x + (double) player.width / 2, player.y + 65, Utils.font2, Color.white);
            } else if(gameMS <= 10000){
                    Utils.drawCenteredString(g,"Stage 1", (double) getGameWidth()/2,(double) getGameHeight()/2,Utils.font1,Color.YELLOW);
            }
            printHUD(g);
        ...
        // Window border and ugly problem solver
        g.setColor(Color.black);
        g.fillRect(getGameWidth(), 0, getGameWidth(), getGameHeight());
        g.setColor(Color.white);
        g.fillRect(0, 0, 2, getGameHeight());
        g.fillRect(getGameWidth()-1, 0, 2, getGameHeight());
